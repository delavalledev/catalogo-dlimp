...
function lerProdutosOnline() {
    ...
    if (!fs.existsSync(arquivo)) {
        throw new Error("Arquivo produtos-online.json não encontrado.");
    }
...
function salvarProdutosOnline(arquivo, dados) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/excluir") {
    ...
    try {
        ...
        /*
         * Descobre onde está o array de produtos
         * dentro do produtos-online.json.
         */

        if (
            Array.isArray(catalogo) &&
            catalogo.length === 1 &&
            Array.isArray(catalogo[0]?.value)
        ) {

            produtos = catalogo[0].value;

            salvar = () => {

                catalogo[0].value = produtos;
                catalogo[0].Count = produtos.length;
            ...
            catalogo &&
            Array.isArray(catalogo.value)
        ) {

            produtos = catalogo.value;

            salvar = () => {

                catalogo.value = produtos;
                catalogo.Count = produtos.length;
            ...
            catalogo &&
            Array.isArray(catalogo.produtos)
        ) {
        ...
        } else if (Array.isArray(catalogo)) {
         ...
         * IMPORTANTE:
         *
         * NÃO alteramos cadproduto.
         * NÃO alteramos ForaDeUso.
         * NÃO fazemos DELETE no Sys-On aqui.
         *
         * O sincronizador cuidará disso.
         */
        ...
        return responder(res, 200, {
            ...
            removidoDoCatalogo: true,

            mensagem:
                "Produto removido do catálogo com sucesso."
        });
